...
- [x] ~~基于人脸识别定位裁剪素人系列的海报~~
  ...
  由于使用默认数据集时OpenCV的检测效果不好，特别是还有FC2等素人影片加在封面上的各种奇怪水印、爱心、马赛克、黑条，以及五花八门的图片角度……（能不能正经点拍照！怒！！）
  
  所以虽然当前版本已经实装了人脸识别，但是可用性还很差，所以配置文件中默认禁用了此功能。
  
  但是目前版本依然包含了一些稳定性方面的更新，以及适配浏览器机制和部分站点抓取器的更新（Chrome和Edge的Cookies机制有更新），新版本应该还是会比之前的好用一点。
  
  [可以来这里下载现在的版本](https://github.com/Yuukiy/JavSP/actions/workflows/pyinstaller.yml)
- [x] 自动检查和更新新版本
- [x] 翻译标题和剧情简介
- [ ] 匹配本地字幕
- [ ] 使用小缩略图创建文件夹封面
